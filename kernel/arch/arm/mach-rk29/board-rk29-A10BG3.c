...
#define CONFIG_SENSOR_0                 RK29_CAM_SENSOR_OV7675                      /* back camera sensor */
...
#define CONFIG_SENSOR_1 RK29_CAM_SENSOR_OV7675                      /* front camera sensor */
...
//#define FB_LCD_STANDBY_PIN          RK29_PIN6_PD1  //test and 4 layer pcb
#define FB_LCD_STANDBY_PIN          RK29_PIN6_PD0 //for 6 layer pcb
...
#if defined (CONFIG_I2C_ROHM)

#define TOUCH_POWER_PIN RK29_PIN0_PD7
#define TOUCH_RESET_PIN RK29_PIN6_PC3
#define TOUCH_INT_PIN   RK29_PIN0_PA2

static void rohm_hw_init(void)
{
	int ret;
	...
	if(TOUCH_POWER_PIN != INVALID_GPIO){
		ret = gpio_request(TOUCH_POWER_PIN, "rohm power pin");
		if(ret != 0){
			gpio_free(TOUCH_POWER_PIN);
			printk("%s: rohm power pin request err\n", __FUNCTION__);
		}
		else{
			gpio_direction_output(TOUCH_POWER_PIN, GPIO_LOW);
			gpio_set_value(TOUCH_POWER_PIN, GPIO_LOW);
		}
	}

	if(TOUCH_RESET_PIN != INVALID_GPIO){
		ret = gpio_request(TOUCH_RESET_PIN, "rohm reset pin");
		if(ret != 0){
			gpio_free(TOUCH_RESET_PIN);
			printk("%s: rohm reset pin request err\n", __FUNCTION__);
		}
		else{
	...
	if(TOUCH_INT_PIN != INVALID_GPIO){
		ret = gpio_request(TOUCH_INT_PIN, "rohm irq");
		if(ret != 0){
			gpio_free(TOUCH_INT_PIN);
			printk("%s: rohm irq pin request err\n", __func__);
		}
		else{
			gpio_direction_input(TOUCH_INT_PIN);
			gpio_pull_updown(TOUCH_INT_PIN, GPIO_HIGH);
...
static void rohm_hw_power(int on)
{
	if(TOUCH_POWER_PIN != INVALID_GPIO){
		printk("%s: on = %d\n", __FUNCTION__, on);
...
static void rohm_hw_reset(void)
{
	if(TOUCH_RESET_PIN != INVALID_GPIO){
		printk("%s\n", __FUNCTION__);
...
static struct rohm_platform_data rohm_platform_info = {
	.hw_init  = rohm_hw_init,
	.hw_power = rohm_hw_power,
	.hw_reset = rohm_hw_reset,
};
...
static struct i2c_board_info __initdata board_i2c2_devices[] = {
...
#if defined (CONFIG_I2C_ROHM)
	{
		...
		.irq           = TOUCH_INT_PIN,
		.flags         = 0,
		.platform_data = &rohm_platform_info,
	},
#endif
...
static int reset_charge_ic(void) 
{
       //To reset charge ic,  first hight, after 100ms, then low
      // rk29_mux_api_set(GPIO2A5_UART1SOUT_NAME, GPIO2L_GPIO2A5);
       gpio_request(RK29_PIN4_PD5, NULL);
       gpio_direction_output(RK29_PIN4_PD5, GPIO_HIGH);
       mdelay(500);
       gpio_direction_output(RK29_PIN4_PD5, GPIO_LOW);
       gpio_free(RK29_PIN4_PD5);       
       return 0;
}

struct rk2918_battery_platform_data rk2918_battery_platdata = {
        .reset_charge_ic = reset_charge_ic,//archos request
	.dc_det_pin      = RK29_PIN4_PA1,
...
#define PWM_EFFECT_VALUE  0 //  4.0 should use 0
//#define PWM_EFFECT_VALUE  1//值越大，越暗
...
//#define BL_EN_PIN         RK29_PIN6_PD0//test and 4 layer pcb
#define BL_EN_PIN         RK29_PIN6_PD1//add for 6 layer pcb
...
static int rk29_backlight_pwm_suspend(void)
{
	...
	//gpio_direction_output(PWM_GPIO, GPIO_HIGH);//changed by hjc
   #ifdef  LCD_DISP_ON_PIN
    gpio_direction_output(BL_EN_PIN, 0);
    gpio_set_value(BL_EN_PIN, !BL_EN_VALUE);
   #endif
...
static int rk29_backlight_pwm_resume(void)
{
    ...
    #ifdef  LCD_DISP_ON_PIN
    msleep(30);
   gpio_direction_output(BL_EN_PIN, 1);//1//1-0 by hjc
    gpio_set_value(BL_EN_PIN, BL_EN_VALUE);
    #endif
	return 0;
}
