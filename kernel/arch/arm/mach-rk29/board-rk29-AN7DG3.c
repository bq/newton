...
#if defined(CONFIG_GOODIX_CAPACITIVE_TOUCHSCREEN) || \
	defined(CONFIG_GOODIX_CAPACITIVE_TOUCHSCREEN_TR719) || \
	defined(CONFIG_GOODIX_CAPACITIVE_TOUCHSCREEN_TR810)
...
#define TOUCH_SCREEN_STANDBY_PIN          INVALID_GPIO//RK29_PIN6_PD1
#define TOUCH_SCREEN_STANDBY_VALUE        GPIO_HIGH
#define TOUCH_SCREEN_DISPLAY_PIN          RK29_PIN6_PD3
#define TOUCH_SCREEN_DISPLAY_VALUE        GPIO_LOW //使能上电
...
*****************************************************************************************/
#define FB_ID                       0
#define FB_DISPLAY_ON_PIN           INVALID_GPIO// RK29_PIN6_PD0 背光定义
#define FB_LCD_STANDBY_PIN          RK29_PIN6_PD1// RK29_PIN6_PD1//INVALID_GPIO
...
int rk29_fb_io_enable(void)
{
	...
	struct regulator *tps65910_vaux2;
	
	tps65910_vaux2 = regulator_get(NULL, "vaux2");
	if (IS_ERR(tps65910_vaux2)) {
		pr_err("fail to get regulator tps65910_vaux2: %ld\n", PTR_ERR(tps65910_vaux2));
		tps65910_vaux2 = NULL;
	}
	else{
		printk("enable tps65910_vaux2\n");
		regulator_enable(tps65910_vaux2);
		regulator_put(tps65910_vaux2);
		tps65910_vaux2 = NULL;
	}
#endif
...
int rk29_fb_io_disable(void)
{
	...
	if (IS_ERR(tps65910_vaux2)) {
		...
		printk("disable tps65910_vaux2\n");
		regulator_disable(tps65910_vaux2);
		regulator_put(tps65910_vaux2);
		tps65910_vaux2 = NULL;
	}
#endif
...
/*touchplus TX series touch*/
#if defined(CONFIG_TOUCHSCREEN_TOUCHPLUS_IIC)
...
static struct touchplus_platform_data touchplus_info = {
        .model   = 801,
        .x_max   = TOUCH_MAX_X,
        .y_max   = TOUCH_MAX_y,
        .hw_init = touchplus_hw_init,
        .reset = touchplus_hw_reset,
};
#endif
...
static int nt11003_init_platform_hw(void)
{
	
     gpio_pull_updown(RK29_PIN6_PD3, 1);
    
     gpio_direction_output(RK29_PIN6_PD3, 0);
     msleep(100);
     gpio_set_value(RK29_PIN6_PD3,GPIO_LOW);    
	   msleep(100);
    if(gpio_request(TOUCH_RESET_PIN,NULL) != 0){
      gpio_free(TOUCH_RESET_PIN);
      printk("p1003_init_platform_hw gpio_request error\n");
       if(gpio_request(RK29_PIN0_PA2,NULL) != 0)
      {
      	gpio_free(RK29_PIN0_PA2);
        printk("ft5x0x_init_platform_hw RK29_PIN0_PA2 gpio_request error3333\n");
      }
      return -EIO;
      }
...
static struct nt11003_platform_data nt11003_info = {
    .model= 5006,
    .init_platform_hw= nt11003_init_platform_hw,
...
int ft5406_init_platform_hw(void)
{
    if(gpio_request(TOUCH_RESET_PIN,NULL) != 0){
      gpio_free(TOUCH_RESET_PIN);
      printk("ft5406_init_platform_hw  1  gpio_request error\n");
      return -EIO;
    }

    if(gpio_request(TOUCH_INT_PIN,NULL) != 0){
      gpio_free(TOUCH_INT_PIN);
      printk("ft5406_init_platform_hw  2   gpio_request error\n");
      return -EIO;
    }
    gpio_pull_updown(TOUCH_INT_PIN, 1);
    gpio_direction_output(TOUCH_RESET_PIN, 0);
...
#define GS_IRQ_PIN RK29_PIN0_PA3
...
static int rk29_backlight_io_deinit(void)
{
    int ret = 0;
    #ifdef  LCD_DISP_ON_PIN
    gpio_set_value(BL_EN_PIN, 1-BL_EN_VALUE);
    gpio_free(BL_EN_PIN);
    #endif
...
#if defined(CONFIG_BATTERY_RK2918) 
/*
系统深度休眠后，放置时间过长，电池电量太低
会唤醒系统，检测电量，然后决定是否自动关机
*/
// RK29_PIN6_PB4
//不是 你插入DC就会充电
//王仪平  16:32:13
...
struct rk2918_battery_platform_data rk2918_battery_platdata = {
	...
	.batt_low_pin    = RK29_PIN4_PA2,
	.charge_set_pin  = RK29_PIN6_PB4,// 检测到充电器，设置为高，平时为低  仅为给系统供电
	.charge_big_charge = RK29_PIN6_PB7,
	.charge_ok_pin   = RK29_PIN4_PA3,

	.batt_low_level = GPIO_LOW,
	.dc_det_level     = GPIO_HIGH,
	.charge_set_level = GPIO_HIGH,
	.charge_big_level = GPIO_HIGH,
	.charge_ok_level  = GPIO_HIGH,
};
 ...
 * author: cmc@rock-chips.com
 *****************************************************************************************/
#define XPT2046_GPIO_INT           RK29_PIN0_PA2
...
static struct cpufreq_frequency_table freq_table[] = {
	...
	{ .index = 1275000, .frequency =  816000 },
...
static void __init machine_rk29_board_init(void)
{
	...
	ret = gpio_request(RK29_PIN4_PA2, NULL);
	if (ret) {
		printk("%s....failed to request batt_low    gpio...........error...\n",__FUNCTION__);		
	}

	if ((gpio_get_value(RK29_PIN4_PA2) == GPIO_LOW) && (gpio_get_value(RK29_PIN0_PA0)))
	 {
	 printk("%s: low battery so powerdown\n",__FUNCTION__);			 
	 while(1)			 
	 {
	 ...
	 printk("%s: low battery so powerdown this a big error\n",__FUNCTION__);
 	}
         usb_flag_detect = dwc_otg_check_dpdm();
        printk("%s..........dwc_otg_check_dpdm=%d\n",__FUNCTION__,usb_flag_detect);
