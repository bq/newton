...
static void rk29_bl_set_brightness(struct backlight_device *bl)
{
...
static int rk29_bl_update_status(struct backlight_device *bl)
{
	...
	brightness = rk29_bl_info->min_brightness + 
	            ((BL_STEP - rk29_bl_info->min_brightness) * brightness) / BL_STEP;

	bl->props.brightness = brightness;

	rk29_bl_set_brightness(bl);
...
static int rk29_bl_get_brightness(struct backlight_device *bl)
{
	...
	if (ref) {
		brightness = BL_STEP*divh/div_total;
	} else {
		brightness = BL_STEP-(BL_STEP*divh/div_total);
	}
...
{		
	...
	if (rk29_bl_info->min_brightness < 0 || rk29_bl_info->min_brightness >= BL_STEP)
		rk29_bl_info->min_brightness = 80;
