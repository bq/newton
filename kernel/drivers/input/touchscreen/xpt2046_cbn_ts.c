...
{
	xpt2046printk("%s:(%d/%d),(x:%d~%d),(y:%d~%d),%d\n",__FUNCTION__,*x, *y,
	ts->x_min,ts->x_max,ts->y_min,ts->y_max,ts->touch_virtualkey_length);
...
//        goto out;
	...
	if (Rt) {
		...
		xpt2046printk("*************:x = %d,y = %d\n",x,y);
		TouchPanelCalibrateAPoint(x, y, &cal_x, &cal_y);
        xpt2046printk("*************:cal_x = %d,cal_y = %d\n",cal_x,cal_y);
    	cal_x = cal_x / 4;
    	cal_y = cal_y / 4;
		gADPoint.x = x;
        gADPoint.y = y;
...
//			swap(cal_x, cal_y);	
		...
		input_report_abs(input, ABS_X, cal_x);//7
#if 0
		if (cal_y > 300)
			cal_y += (cal_y-230)/10;
...
#endif
#if 1
 if ((ABS(last_x - cal_x) < 2) || (ABS(last_y - cal_y) < 2))
	goto out;
#endif
	last_x = cal_x;
	 last_y = cal_y;

		input_report_abs(input, ABS_Y, cal_y);//10
	...
	if (ts->read_cnt==1 || (abs(ts->last_read - *val) > ts->debounce_tol)) {
		...
		if (++ts->read_rep >= ts->debounce_rep) {
			...
			*val = average_val[data_idx]/(ts->debounce_rep);
			xpt2046printk("***>%s:XPT2046_FILTER_OK\n",__FUNCTION__);
...
#endif
    printk("xpt2046_ts: driver initialized...yj......cbn..........yjyjyjyj\n");
	return 0;
...
static int  xpt2046_shutdown(struct spi_device *spi)
{
	int ret = 0;
	ret = xpt2046_suspend(spi,PMSG_SUSPEND);
...
device_initcall_sync(xpt2046_init);
