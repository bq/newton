...
#endif
#define TS_MAXX 800//800  //changed by hjc
#define TS_MAXY 480
	...
	if(fingerbuf[0] != 0){
		...
		//	printk("%d: (%d,%d,%d)\n", fingerid, x, y,posp[4]);
			//if((x!=0)&&(y!=0)){
				...
				input_mt_slot(tsd->input_dev, fingerid-1);
...
/*read fireware judge if ok*/
#if 0
	ret = touchplus_read_regs(client,52,buf,3);
	if(ret < 0)
	{
		printk("touchplus_i2c_ts_probe:i2c_transfer fail =%d reg=%d\n",ret,52);
		touchplus_hw_remove();
		return ret;
	}
	else
		printk("%s.read firwware=%s\n",__FUNCTION__,buf);
#endif
/*fireware read messy code so another choice*/

	ret = touchplus_read_regs(client,57,buf,1);
	if((ret <  0) || ((buf[0] != 26) && (buf[0] != 27)))
	{
